...
2. **Búsqueda híbrida (keywords):** `embed_text(query, input_type="query")` → `hybrid_search(session, query, q_emb, k)`: búsqueda densa (pgvector `<=>`, coseno, índice HNSW `ix_kag_chunks_embedding` de la migración `0015_kag_hnsw`) + búsqueda léxica FTS (`ts_rank_cd` sobre `content_tsv`, config `'simple'`) fusionadas con **RRF** (`rrf_merge`, k=60). K = `top_k` (8) local, `global_top_k` (20) global. Si `embed_text` falla (p. ej. sin `embedding_settings` activa) → `q_emb=None` y `hybrid_search` degrada a **solo FTS** (rollback + log). Si la migración `0014_kag_fts` no está aplicada → degrada a solo búsqueda densa (rollback + log). Si todo falla → `session.rollback()` (la transacción queda abortada tras el error) y `vec_hits = []` (degradación natural). Después, `apply_relevance_threshold` descarta la cola larga: score < `MIN_SCORE_RATIO` × max_score o < `MIN_ABS_SCORE` (escala-agnóstico: funciona con RRF, coseno o ts_rank).
   2.5. **LLM crítico → búsqueda textual** (`critic_regex_search`): el crítico (modelo pequeño) decide si la pregunta contiene términos EXACTOS (nombres propios, países, códigos alfanuméricos como CVE-2024-3094, acrónimos, fechas, cifras) y devuelve `(hits, terms)`. Los `hits` son chunks por FTS por término (config `'simple'`, alta precisión); los `terms` también alimentan el entity linking + PPR del paso 3 — el grafo se aplica sobre las keywords de la pregunta Y sobre las del crítico. Si el LLM falla → heurística determinista (`_deterministic_regex_terms`: tokens con mayúscula inicial o códigos alfanuméricos). Sin términos → `([], [])` y el flujo normal sigue.
3. **Entity linking anclado + copresencia** (sobre `query + regex_terms`, una sola llamada LLM):
...
5. **Merge + dedup (RRF 3 capas):** `chunks_for_entities(ppr_entities)` → `rrf_merge(vec_hits, regex_hits, ppr_chunks, k=60)` → una sola lista de chunks ancla, sin duplicación (cada capa aporta su rank; RRF es escala-agnóstico: ts_rank, coseno y menciones no comparten escala). Luego `_group_chunks_with_window`: cada ancla se expande con sus ±`CONTEXT_WINDOW` vecinos del mismo doc (agrupador de chunks consecutivos), ordenado por importancia del ancla y capado en `MAX_CONTEXT_CHUNKS`.
...
9. **Ensamblar contexto** (`assemble_context`):
   ...
   ```
   `history` (opcional) se incluye como sección informativa al inicio (preparado, sin probar con Docker).
...
Seis optimizaciones implementadas sobre el flujo base de §3.1:
...
**PPR (`PPR_MIN_RATIO`, `PPR_Z`).** Con alpha=0.15, cada salto en el grafo decae ~×0.15: la semilla puntúa ~(1-α)/|seed|, los vecinos a 1 salto ~α×eso, los de 2 saltos ~α²×eso. `PPR_MIN_RATIO = 0.02` ≈ semilla + 2 saltos (0.15² = 0.0225): captura la vecindad local natural para razonamiento multi-hop sin arrastrar la cola. Para calibrar: correr queries representativas con `verbose=True` (imprime el top-5 PPR), loguear la distribución completa de scores y ajustar `PPR_MIN_RATIO` al codo natural (el punto donde la cola power-law se aplana). La guarda estadística (`mean + PPR_Z × std`) solo actúa en grafos grandes (n ≥ 100): en grafos pequeños la media es significativa y sobre-filtra. `PPR_Z = 0.5` es conservador (mantiene bastante); subirlo si entran demasiadas entidades irrelevantes, bajarlo si se pierden entidades relevantes.

**Desambiguación (`DISAMBIG_MIN_OVERLAP`, `DISAMBIG_MARGIN`).** El overlap coefficient normaliza por grado: un candidato de grado 100 con 5 vecinos compartidos NO es más cercano que uno de grado 3 con 3 compartidos. `DISAMBIG_MIN_OVERLAP = 0.1` es un piso bajo (filtra solo candidatos sin señal); el discriminador real es `DISAMBIG_MARGIN = 0.05` (el mejor debe superar al segundo por 5 puntos de overlap). Para calibrar: loguear `(best_score, second_score, gap)` de cada mención ambigua y ajustar el margen — subirlo si la desambiguación elige mal (cae al primer candidato, el default ambiguo), bajarlo si deja demasiadas menciones sin resolver.

**Canonicalización (`DEFAULT_SIM_THRESHOLD`, `DEFAULT_GAP_MARGIN`).** En la ingesta, `_canonicalize` fusiona variantes superficiales por coseno ≥ 0.92 con margen ≥ 0.03 sobre la segunda mejor. El umbral es ALTO a propósito: fusionar conceptos distintos es peor que dejar variantes separadas. El margen evita fusiones a ciegas cuando dos representantes están igual de cerca (empate → la variante se queda separada). Para calibrar: loguear `(best_sim, second_sim, gap)` durante la ingesta; si el grafo queda fragmentado (demasiadas variantes), bajar el umbral o el margen; si se fusionan conceptos distintos, subirlos.

**Relevancia (`MIN_SCORE_RATIO`, `MIN_ABS_SCORE`).** `MIN_SCORE_RATIO = 0.15` descarta la cola larga de la búsqueda híbrida (relativo al mejor score); `MIN_ABS_SCORE = 0.001` es el seguro contra queries sin señal. Con RRF (k=60), el mejor score es ~1/61 ≈ 0.016 y el piso relativo ~0.0024 — el piso absoluto de 0.001 queda por debajo, así que domina el relativo.
...
| `src/kag_query.py`                  | `_fix_db_host`, `classify_query`, `_deterministic_entity_fallback`, `_noun_chunk_fallback`, `_get_spacy_nlp`, `_spacy_model_for`, `grounded_entity_linking`, `match_entities_candidates`, `disambiguate_by_cooccurrence`, `build_adjacency`, `personalized_pagerank`, `ppr_entity_selection`, `vector_search`, `fts_search`, `rrf_merge` (variádico), `hybrid_search`, `chunks_for_entities`, `subgraph_triples`, `figures_for_chunks`, `doc_summaries`, `apply_relevance_threshold`, `expand_chunk_window`, `_group_chunks_with_window`, `_deterministic_regex_terms`, `critic_regex_search`, `assemble_context`, `generate_answer`, `ask`, `main` |
| `tests/test_kag.py`                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `tests/test_kag.py` | 58 tests de lógica pura (sin DB, sin torch/spacy): chunking con segmenter fake (incl. fusión hasta max_tokens y `use_coref`), PPR, clasificación, ensamblado (incl. `history`), normalización, detección de idioma, `complete_local` (monkeypatch httpx), RRF (2 y 3 capas), `ppr_entity_selection` (umbral relativo + guarda estadística), desambiguación por copresencia (overlap coefficient + margen), entity linking anclado, crítico regex (terms + heurística), `hybrid_search` (degradación y merge) |     |
