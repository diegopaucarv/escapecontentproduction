...
- El LLM es la opción SIEMPRE presente: la decisión se orquesta por LLM y
  se resuelve con el SLM (modelo pequeño). El scoring determinista
  (src/agents/novelty_router.py::score_novelty) sigue existiendo como
  respaldo, pero degradar a él es determinista y SIEMPRE requiere
  aceptación del usuario (requires_user_acceptance=True).
...
- Degradación elegante: si el LLM no está disponible, se asume ángulo
  nuevo (comportamiento determinista actual), marcado como degradado y
  pendiente de aceptación del usuario.
"""
